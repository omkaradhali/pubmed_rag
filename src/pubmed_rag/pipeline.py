...
from pubmed_rag.chunk import chunk_records, load_and_chunk
...
class SourceChunk:
    ...
    pmid: str
    title: str
    authors: list[str]  # ["LastName Initials", ...]; empty list if unavailable
    journal: str
    year: str
...
def _run_full_ingest(reldate: int | None = None) -> None:
    """Wipe existing corpus and rebuild: ingest → chunk → embed → seed."""
    for path in (ABSTRACTS_PATH, EMBEDDINGS_PATH):
        if path.exists():
            path.unlink()
            _logger.info("Removed stale %s", path)

    # Wipe chroma_db so the collection starts clean — upsert alone won't remove
    # chunks from PMIDs that no longer exist in the new corpus.
    ...
    _logger.info("Produced %d chunks.", len(chunks))

    embedded = embed_chunks(chunks)
    save_embeddings(embedded, EMBEDDINGS_PATH)
    _logger.info("Embedded %d chunks → %s", len(embedded), EMBEDDINGS_PATH)

    n_seeded = seed_collection(EMBEDDINGS_PATH)
    _logger.info("Seeded %d chunks into ChromaDB.", n_seeded)
...
def _run_incremental_update(reldate: int) -> None:
    ...
    _logger.info("Upserted %d new chunks into ChromaDB.", n_upserted)
